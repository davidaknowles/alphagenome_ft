...
def test_zemke2024_donor_smoke_recovers_metadata_molecules(tmp_path):
    ...
    assert result == {
        "donor": "hc1",
        "cells": 2,
        "raw_gene_expression_molecules": 10,
        "metadata_nCount_RNA": 10,
        "raw_molecule_excess": 0,
        "relative_raw_molecule_excess": 0.0,
        "gene_features": 2,
        "valid_groups": 1,
        "nonempty_groups": 1,
    }
...
def test_raw_molecule_excess_enforces_relative_limit():
    with pytest.raises(ValueError, match="above the 0.1000% limit"):
        audit_raw_molecule_excess(
            np.asarray([1002.0]),
            np.asarray([1000]),
            donor="hc1",
            maximum_relative_excess=0.001,
        )


def test_raw_molecule_excess_rejects_counts_below_metadata():
    with pytest.raises(ValueError, match="below metadata nCount_RNA"):
        audit_raw_molecule_excess(
            np.asarray([999]),
            np.asarray([1000]),
            donor="hc1",
            maximum_relative_excess=0.1,
        )
