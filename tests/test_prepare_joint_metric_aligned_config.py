...
def test_prepare_config_can_set_separate_zemke_rna_weight(tmp_path: Path) -> None:
    datasets = []
    for dataset, policy in CORRELATION_WEIGHTS.items():
        target_path = tmp_path / "source" / dataset / "targets.json"
        target_path.parent.mkdir(parents=True)
...
def test_prepare_config_can_set_separate_zemke2024_rna_weight(tmp_path: Path) -> None:
    datasets = []
    for dataset, policy in CORRELATION_WEIGHTS.items():
        target_path = tmp_path / "source" / dataset / "targets.json"
        target_path.parent.mkdir(parents=True)
    ...
    assert weights["zemke2024"] == {"zemke2024_all_rna": 1.0}
