...
`compare_released.py` compares either reconstructed broad-subclass coverage or Tn5 insertion SPMR to the matching released broad BigWigs at the native 100 bp bin size. It reports ordinary and signed double-centered correlation across tracks and genomic bins, plus per-subclass signal scale and correlation. A one-donor comparison checks the pipeline, while the all-donor reconstruction is the decision-making comparison because the released BigWigs pool the full cohort.
