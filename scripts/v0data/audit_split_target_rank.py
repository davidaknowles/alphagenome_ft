#!/usr/bin/env python3
"""Compare direct RNA target complexity across held-out chromosomes."""
...
def audit_chromosomes(
    ...
    *,
    ...
    return {
        ...
        "support": "fixed_window_full_span" if fasta_index_path else "all_supervision_genes",
...
def render_markdown(result: dict[str, object], dataset_label: str) -> str:
    lines = [
        f"# {dataset_label} held-out chromosome RNA target rank",
        "",
    ...
    for scale, title in (
        ("raw_cpm", "Raw counts per million"),
...
def main(*, default_dataset_label: str = "Dataset") -> None:
