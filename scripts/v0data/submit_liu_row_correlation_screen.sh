#!/bin/bash
# Submit a matched one-epoch Liu row-centered RNA objective screen.
...
suffix="${weight//./p}"
targets="outputs/v0data/liu-hdma/joint/targets_geneonly_rowcorrw${suffix}.json"
...
exports="ALL,DATASET=liu-hdma,TARGETS_CONFIG=${targets},RUN_SUFFIX=_geneonly_rowcorrw${suffix}_screen,NUM_EPOCHS=1,BALANCE_GENE_WINDOWS=1"
...
printf 'Liu row-correlation weight %s smoke=%s full=%s\n' "$weight" "$smoke" "$full"
