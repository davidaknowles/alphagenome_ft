...
Macaque fragment files use library-local 10x barcodes while the comprehensive metadata uses globally unique identifiers. Each file is assigned to a metadata library only when one library covers every fragment barcode and is the unique best match. Marmoset stores 112 full fragment files and 112 H5AD-filtered exact subsets of those files. Reconstruction uses only the filtered copies to avoid double-counting; their retained cells have fragment counts identical to the corresponding full resources.
