...
The assay for transposase-accessible chromatin, ATAC, tracks are cell-group pseudobulk BigWigs. Fragment resources are tabix-indexed five-column records with chromosome, start, end, cell identifier, and multiplicity; per-cell whole-genome fragment histograms are stored alongside them. Marmoset has 112 distinct libraries, each represented by a full fragment resource and an H5AD-filtered exact subset, yielding 224 resource directories but not 224 independent libraries. Reprocessing uses the filtered copy because it retains every fragment for cells in the comprehensive metadata and avoids double-counting the same cells. The comprehensive expression matrices use the AnnData Hierarchical Data Format 5, H5AD, format and contain cell identifiers, group assignments, normalized expression, and raw unique molecular identifier counts. Group pseudobulks must sum raw counts before counts-per-million normalization. Summing per-cell normalized expression produces materially different raw cell-group structure and is not used for corrected training targets. Additional resources include aligned 10x multiome matrices, expression metadata, taxonomy mappings, spatial-expression data from MERSCOPE and Xenium platforms, macaque Patch-seq data, and supplementary tables.
...
The count-matrix subclass metadata also lacks the finer `L5 IT` assignment represented by a published track, although it contains `L4/5 IT`. Because the H5AD observation tables contain no lower-level cluster annotation, count-derived direct-gene supervision supports 19 of the 20 published channels. The `L5_IT` count channel must be masked rather than inferred from `L4/5 IT`; its published coordinate-resolved track remains usable.
