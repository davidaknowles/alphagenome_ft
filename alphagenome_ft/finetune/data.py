...
class WindowedTargetCache:
    ...
    def __init__(
        ...
        *,
        ...
        cache_splits: Sequence[str] | None = None,
    ) -> None:
        self.cache_dir = Path(cache_dir).expanduser().resolve()
        self.dtype = _normalize_cache_dtype(dtype)
        self._intervals = {split: list(values) for split, values in intervals.items()}
        self._cache_splits = self._select_splits(self._intervals, cache_splits)
        self._cached_intervals = {
            split: self._intervals[split] for split in self._cache_splits
        }
    ...
    def build(
        ...
        *,
        ...
        cached_intervals = cls._select_interval_mapping(intervals, cache_splits)
        manifest_path = cache_dir / "manifest.json"
        manifest = cls._make_manifest(cached_intervals, head_specs, dtype_np)
        ...
        for split, split_intervals in cached_intervals.items():
    ...
    def _select_splits(
        ...
        available = tuple(intervals)
        requested = available if cache_splits is None else tuple(cache_splits)
        if not requested:
            raise ValueError("Target cache must include at least one split.")
        duplicates = len(set(requested)) != len(requested)
        unknown = [split for split in requested if split not in intervals]
        if duplicates or unknown:
            details = []
            if duplicates:
                details.append("duplicate split name")
            if unknown:
                details.append(f"unknown split(s) {unknown}")
            raise ValueError(
                "Invalid target-cache splits, "
                + "; ".join(details)
...
- Arrays: `<cached_split>/<head_id>.npy`
...
"""
    ...
    def _validate_manifest(self) -> None:
        expected = self._make_manifest(self._cached_intervals, self._head_specs, self.dtype)
        if self._manifest != expected:
            raise ValueError(
                f"Target cache manifest does not match requested intervals/tracks: "
                f"{self._manifest_path}"
            )
        for split, split_intervals in self._cached_intervals.items():
    ...
    def has_split(self, split: str) -> bool:
        return split in self._cache_splits

    def arrays_for_split(self, split: str) -> dict[str, np.memmap]:
        if not self.has_split(split):
            raise ValueError(f"Target cache does not include split {split!r}.")
...
class BigWigDataModule:
    ...
    def __init__(
        ...
        *,
        ...
        target_cache_splits: Sequence[str] | None = None,
        ...
        Args:
            ...
            target_cache_splits: Optional subset of split names stored in the
                target cache. Other splits continue to stream BigWig targets.
        ...
        self._target_cache = (
            WindowedTargetCache(
                ...
                cache_splits=target_cache_splits,
            )
    ...
    def iter_batches(
        ...
        with contextlib.ExitStack() as stack:
            target_cache_arrays = (
                ...
                if self._target_cache is not None and self._target_cache.has_split(split)
                else None
            )
