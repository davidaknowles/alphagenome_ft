...
The first matched direct-gene result supports the representation change for Zemke 2023. With the backbone frozen and both heads trained for one epoch, direct raw-UMI CPM gene supervision raises Zemke validation RNA $R$ from 0.2233 to 0.3363 and test RNA $R$ from 0.1269 to 0.3765 relative to the coordinate-RPKM warmup. Zemke ATAC and every other dataset head remain effectively unchanged. This is evidence for a more comparable RNA target, not an improvement on the released coordinate-track endpoint; the full warmup and matched LoRA versus LoRA plus LoCon branches remain in progress.
