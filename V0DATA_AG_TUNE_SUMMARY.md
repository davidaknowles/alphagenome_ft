...
Expanding LoCon coverage from downsampling blocks 4 and 5 to blocks 2 through 5 gives a modest aggregate improvement. Validation $R$ rises from 0.5934 to 0.5984 and test $R$ from 0.5851 to 0.5897. The validation gain is ATAC-led and dominated by Zemke 2023, while the test gain is RNA-led; broader LoCon coverage is therefore useful but does not solve the RNA ceiling. Increasing LoRA and LoCon rank has not improved the source model. Equal native-source sampling is the complementary strategy for testing whether multi-species studies are underexposed under equal study weighting.
