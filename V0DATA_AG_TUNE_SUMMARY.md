...
Expanding LoCon coverage from downsampling blocks 4 and 5 to blocks 2 through 5 initially regressed, then exceeded the source after the predeclared patience was retained. Epoch thirty-seven raises aggregate validation $R$ from 0.59337 to 0.59842 and mean validation ATAC from 0.67928 to 0.69106, while mean RNA changes from 0.50746 to 0.50578. Aggregate test $R$ also rises from 0.58509 to 0.58974, with unchanged mean ATAC and RNA increasing from 0.45038 to 0.45972. The validation improvement is dominated by Zemke 2023 ATAC and therefore requires continued held-out scrutiny. Higher rank has not improved the source. A separate source-balanced continuation tests whether equal study weighting underexposes the three Johansen and four Zemke 2023 native genomes; it changes only training order and per-source update counts while preserving every target manifest.
...
An optional modality-separated update schedule is prepared for the all-direct-gene, source-specific endpoint. The measured ATAC/RNA shared-adapter gradient cosine similarities are positive, from 0.078 to 0.335, so projected-gradient conflict surgery is not supported by the diagnostics. Their norms are nevertheless imbalanced, reaching about ninefold for Zemke 2023 human ATAC versus RNA. The schedule therefore repeats each training genomic batch once per active head and applies independent optimizer updates, preserving full ATAC and RNA coverage and leaving validation and test unchanged. It costs about twice as much training compute and remains an unselected experimental variant until measured against the joint-head schedule.

The matched direct-gene result supports the representation change for Zemke 2023. With the backbone frozen for three epochs, direct raw-UMI CPM gene supervision raises Zemke validation RNA $R$ from 0.2496 to 0.3601 and test RNA $R$ from 0.1520 to 0.4207 relative to the coordinate-RPKM warmup. Zemke ATAC and every other dataset head remain effectively unchanged. This is evidence for a more comparable RNA target, not an improvement on the released coordinate-track endpoint; both optimizer-preserving warmup continuations and their matched LoRA versus LoRA plus LoCon branches remain in progress.
