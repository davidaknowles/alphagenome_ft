...
LoCon gives a useful improvement over LoRA alone. Uniformly doubling RNA loss weight does not help. Metric-aligned objectives provide the largest gain, especially for RNA, without reducing aggregate ATAC performance. Lower-rate consolidation changes the result only marginally, so the metric-aligned and tempered models should be treated as effectively tied rather than as evidence for a further material gain.

## Dataset difficulty

The table reports the nominal tempered model. Values are validation/test $R$.
...
The repeat-overlapping coarse bin accounts for 70.7% of chromosome-9 double-centered RNA target variance. A matched diagnostic changed only genomic support by omitting the single 131 kb window containing the repeat. RNA $R$ increased from 0.1570 to 0.6291, while ATAC changed from 0.6046 to 0.6156. The assembly and track chromosome sizes agree, and the low RNA value is stable across model variants, so neither model selection nor assembly routing explains the extreme canonical value. The repeat-filtered result remains below 0.8, but it better represents broader chromosome-9 performance.
...
No broadly successful RNA configuration has reached $R\approx0.8$. Post hoc refitting of only the dataset heads does not improve the aggregate validation correlation and shifts the balance slightly toward ATAC while reducing RNA. Head-only optimization is therefore unlikely to remove the remaining ceiling by itself.

Expanding LoCon coverage from downsampling blocks 4 and 5 to blocks 2 through 5 gives a modest aggregate improvement. Validation $R$ rises from 0.5934 to 0.5984 and test $R$ from 0.5851 to 0.5897. The validation gain is ATAC-led and dominated by Zemke 2023, while the test gain is RNA-led; broader LoCon coverage is therefore useful but does not solve the RNA ceiling. Increasing LoRA and LoCon rank gives a smaller ATAC-led gain while reducing RNA, so broader rank-four LoCon is preferred. Equal native-source sampling is the complementary strategy for testing whether multi-species studies are underexposed under equal study weighting.

Newly initialized dataset heads are warmed against the frozen pretrained backbone before adapter training. Matched LoRA and LoRA plus LoCon models then start from the same warmed heads with zero adapter residuals. This prevents random projections from driving the first shared-backbone updates and isolates the effect of adapter strategy.

The multispecies studies currently share output projections across native species even though their target distributions differ. Johansen RNA double-centered standard deviation is 0.00123 in human and about 0.0018 in macaque and marmoset. Zemke 2023 RNA ranges from 3.81 to 7.50 in its released RPKM representation, and its ATAC double-centered standard deviation ranges from 3.21 to 8.59. A source-specific-head formulation therefore assigns an independent ATAC/RNA projection to each Johansen and Zemke 2023 species while retaining the shared backbone, aligned channel order, and equal native-source exposure. This is preferable to treating macaque or marmoset as additional AlphaGenome organism classes because the pretrained trunk supports human and mouse organism rows, while the new distinction is needed at the dataset output layer.
...
An optional modality-separated update schedule is available for the all-direct-gene, source-specific endpoint. The measured ATAC/RNA shared-adapter gradient cosine similarities are positive, from 0.078 to 0.335, so projected-gradient conflict surgery is not supported by the diagnostics. Their norms are nevertheless imbalanced, reaching about ninefold for Zemke 2023 human ATAC versus RNA. The schedule therefore repeats each training genomic batch once per active head and applies independent optimizer updates, preserving full ATAC and RNA coverage and leaving validation and test unchanged at about twice the training compute.

The matched direct-gene result supports the representation change for Zemke 2023. Direct raw-UMI CPM gene supervision raises Zemke validation RNA $R$ from 0.2496 to 0.3601 and test RNA $R$ from 0.1520 to 0.4207 relative to coordinate RPKM, while Zemke ATAC and every other dataset head remain effectively unchanged. This is evidence for a more comparable RNA target, not an improvement on the released coordinate-track endpoint.
