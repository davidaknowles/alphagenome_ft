...
- `bf16_triton_conv_no_intermediates_tritonpool_fusedembed_stdconv_effective`: also fuses `encoder.dna_embedder.block` as RMSBatchNorm + JAX GELU + int8 weight-only Conv1d in one Triton kernel.
...
| `bf16_triton_conv_no_intermediates_tritonpool_fusedembed_stdconv_effective` | 7.68 | 32347 | 21735 | 31660 | 0.8101 | 0.7989 |

Run root for the new strategy:

`outputs/quant_ablation/20260701_no_intermediates_tritonpool_batch32_full`

Run root for the fused embedder block strategy:

`outputs/quant_ablation/20260701_fused_embed_batch32_full`
...
| `bf16_triton_conv_no_intermediates_tritonpool_fusedembed_stdconv_effective` | 21684 | 30550 | `encoder.down_blocks.0` |
...
- `outputs/quant_ablation/20260701_mem_attribution/bf16_triton_conv_no_intermediates_tritonpool_fusedembed_batch32_valid1_eval128_encoder_staged`
...
- Fusing the full-resolution DNA embedder block reduces observed memory by another `2.6 GiB`, but throughput drops by about `9%`; use it only when memory headroom matters more than examples/s.
