...
The completed benchmark scores 10,452 of the 10,556 cached records. The
remaining 104 records are variants that could not be represented in the
reference sequence and are excluded consistently from model and native-track
comparisons. Table~\ref{tab:xqtl_performance} reports aggregate performance;
the native series uses one biologically constrained native RNA-seq or CAGE
track selection per XQTL panel. Head-only is the strongest trained model on
the aggregate TSS and gene-span metrics. Native matched tracks are close to
the trained models for gene-span average precision, while distance-stratified
performance is shown in Figures~\ref{fig:xqtl_auroc} and~\ref{fig:xqtl_aupr}.

\begin{table}[ht]
\centering
\scriptsize
\caption{Aggregate XQTL variant-effect performance across the six panels.}
\label{tab:xqtl_performance}
\begin{tabular}{@{}lrrrr@{}}
\toprule
Method & TSS AUROC & TSS AUPR & Gene AUROC & Gene AUPR \\
\midrule
Head-only & 0.564 & 0.514 & 0.658 & 0.626 \\
LoRA & 0.550 & 0.498 & 0.643 & 0.608 \\
LoRA+LoCon & 0.556 & 0.505 & 0.635 & 0.606 \\
Native matched tracks & 0.541 & 0.491 & 0.627 & 0.626 \\
\bottomrule
\end{tabular}
\end{table}
...
\label{fig:xqtl_auroc}
\caption{AUROC by absolute distance from the TSS.}
\end{figure}
}{The distance-stratified AUROC figure is missing.}
...
\end{figure}
}{The distance-stratified AUPR figure is missing.}
