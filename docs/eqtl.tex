...
with numbered panels denoting subpanels. The base AlphaGenome comparator uses
a semantic-bootstrap-initialized copy of the human Zemke 2023 RNA head. The
fine-tuned models retain the same target track definitions. The matching used
for every model is shown below.

\begin{table}[ht]
\centering
\small
\caption{Fine-mapping panel to RNA-track matching. A numbered panel inherits
the mapping for its prefix. The base and fine-tuned models use the same
matched RNA tracks.}
\label{tab:cell_matching}
\begin{tabular}{@{}p{0.16\linewidth}p{0.37\linewidth}p{0.37\linewidth}@{}}
\toprule
Panel prefix & Base AlphaGenome & LoRA and LoCon \\
\midrule
\texttt{Ast*} & ASC & ASC \\
\texttt{End*} & Endo & Endo \\
...
This is the closest available cell-type match because the pretrained model does
not provide native single-cell RNA tracks with the fine-mapping labels.
